...
const SingleSaller = (props) => {
  ...
  useEffect(() => {
    console.log("id", id);
    console.log("productSaller", productSaller);
    let data = productSaller.filter((e) => e.id == id);
  ...
  return (
    <div>
      <Header>
        <h1 className="text-white font-black mt-1 text-lg">
          {stateValue.name}
        </h1>
      </Header>
      ...
      <div style={{ direction: "rtl" }} className="p-4">
        {stateValue.products ? (
          <carousel>
            {" "}
            {stateValue.products[1].map((newVal, i) => {
              return (
